...
const CreateDistributorOrderScreen = () => {
    const navigation = useNavigation();
    const route = useRoute();
    const { distributorId } = route.params as { distributorId: number };
    const { user } = useAuth();
    const { createOrder, loading } = useDistributors();
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            Alert.alert('Success', 'Order created successfully', [
                { text: 'OK', onPress: () => navigation.replace('DistributorOrderDetail', { orderId: response.id }) }
            ]);
        } catch (err: any) {
                ...
                <View style={styles.section}>
                        ...
                        <View key={item.id} style={styles.itemCard}>
                            ...
                            <View style={styles.itemDetails}>
                                ...
                                <View style={styles.itemPrice}>
                                    ...
                                    <Text style={styles.priceValue}>{formatCurrency(item.unitPrice)}</Text>
                                </View>
                    ...
                    {items.length === 0 && (
                        <TouchableOpacity
                            ...
                            onPress={() => setShowProductPicker(true)}
                        >
                            <Icon name="cart-plus" size={48} color={colors.gray[300]} />
                            <Text style={styles.emptyItemsText}>Add items to order</Text>
                        </TouchableOpacity>
                    )}
                </View>
...
export default CreateDistributorOrderScreen;